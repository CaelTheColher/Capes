...
class ToggleMenu(parent: Screen, gameOptions: GameOptions) : MainMenu(parent, gameOptions) {
    ...
    override fun init() {
        ...
        addDrawableChild(ButtonWidget.builder(CapeType.SKINMC.getToggleText(config.enableSkinMC)) {
            config.enableSkinMC = !config.enableSkinMC
            config.save()
            it.message = CapeType.SKINMC.getToggleText(config.enableSkinMC)
